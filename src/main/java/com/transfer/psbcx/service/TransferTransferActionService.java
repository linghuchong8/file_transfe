...
import com.transfer.psbcx.pojo.TransferDataVerifyMessage;
...
import com.transfer.psbcx.util.TransferFileEncodingDetector;
import com.transfer.psbcx.util.TransferFileTools;
...
@Service
@Slf4j
public class TransferTransferActionService {
    ...
    @Autowired
    private TransferTransferPutService putService;
    @Autowired
    private TransferSendFileInformationMapper sendFileMapper;
    @Autowired
    private TransferVerifyContentMapper verifyContentMapper;
     ...
     */
    @Async
    public void sendHdfs(TransferDataShareMessage msg) {
        Preconditions.checkArgument(msg != null, "消息不能为空");
        String fileName = msg.getFileName();
        Preconditions.checkArgument(fileName != null, "文件名不能为空");
        String sendSysCode = msg.getSendSysCode();
        ...
        String srcPath = TransferStringHelper.addEndSeparator(fileDir).replace("D:", "");
        if (fileName.contains(".gz")) {
            fileName = fileName.replace(".gz", "");
        }
        String targetPath = TransferAppPropertiesUtil.getProperty("task.process.hdfs.path.prefix", "/user/hdfs")
     ...
     */
    @Async
    public void process(TransferDataShareMessage msg) {
        Preconditions.checkArgument(msg != null, "消息不能为空");
        String fileName = msg.getFileName();
        Preconditions.checkArgument(fileName != null, "文件名不能为空");
        String sendSysCode = msg.getSendSysCode();
        ...
        try {
            String sourcePath = TransferStringHelper.addEndSeparator(fileDir) + fileName;
            String targetPath = "";
            ...
            if (Objects.equals(extension, "gz")) {
                ...
                TransferFileTools.decompress(sourcePath, xmlPath, true);
                targetPath = xmlPath.replace(".xml", ".txt");
                if (special) {
                    TransferFileTools.xmlSpectxt(xmlPath, targetPath, fileCode, specFile);
                } else {
                    TransferFileTools.xml2txt(xmlPath, targetPath, fileCode);
                }
            } else if (Objects.equals(extension, "dat")) {
                targetPath = sourcePath.replace(".dat", ".txt");
                TransferFileTools.dat2txt(sourcePath, targetPath);
            } else if (Objects.equals(extension, "xml")) {
                targetPath = sourcePath.replace(".xml", ".txt");
                if (FINANCE_SYS_CODE.equals(sendSysCode) && !special) {
                    TransferFileTools.xml2txtForFinance(sourcePath, targetPath, fileCode);
                } else if (special) {
                    TransferFileTools.xmlSpectxt(sourcePath, targetPath, fileCode, specFile);
                } else {
                    TransferFileTools.xml2txt(sourcePath, targetPath, fileCode);
                }
            } else if (Objects.equals(extension, "csv")) {
                targetPath = sourcePath.replace(".csv", ".txt");
                TransferFileTools.csv2txt(sourcePath, targetPath, fileCode);
            ...
            long fileByte = 0;
            String processFlag = TransferFlowStatus.DONE;
            String logMsg = "数据处理成功";
            if (!"".equals(targetPath)) {
                File targetFile = new File(targetPath);
                if (!targetFile.exists() || !targetFile.isFile()) {
                    fileByte = -1;
                    processFlag = TransferFlowStatus.ERROR;
            ...
            params.put("processFlag", TransferFlowStatus.ERROR);
            shareMsgMapper.updateMessage(params);
        } catch (Throwable t) {
            log.error("【{}】 处理异常, 再次重试 : {}", fileName, t.getMessage());
            params.put("logMsg", "数据处理异常, 再次重试");
            params.put("processFlag", TransferFlowStatus.READY);
            shareMsgMapper.updateMessage(params);
        }
    ...
    private void parseVerfFile(String sourcePath, String fileName, String fileCode,
        ...
        List<String> lines = FileUtils.readLines(new File(sourcePath),
                TransferFileEncodingDetector.getJavaEncode(sourcePath, fileCode));
        int lineNo = 0;
        for (String line : lines) {
            ...
            if (!line.contains(".check") && line.trim().length() != 0) {
                ...
                if (fields.length < 3) {
                    ...
                    if (matcher.matches()) {
                        TransferVerifyContent content = new TransferVerifyContent();
     ...
     * @param msg 校验消息
     */
    public void downloadVerify(TransferDataVerifyMessage msg) {
        Preconditions.checkArgument(msg != null, "消息不能为空");
        String fileName = msg.getFileName();
        Preconditions.checkArgument(fileName != null, "文件名不能为空");
        String sendSysCode = msg.getSendSysCode();
        ...
        params.put("downFlag", TransferFlowStatus.RUNNING);
    ...
    private String collectVerfDescription(String sourcePath, String fileName) throws Exception {
        Pattern pattern = Pattern.compile(VERF_LINE_REGEX);
        List<String> lines = FileUtils.readLines(new File(sourcePath),
                TransferFileEncodingDetector.getJavaEncode(sourcePath));
