...
import com.transfer.psbcx.enums.TransferDataFileCategory;
...
@EnableScheduling
@Slf4j
public class TransferReceiveMessageJob implements TransferJob {
    ...
    public TransferReceiveMessageJob() {
        downloadRoot = TransferAppPropertiesUtil.getProperty("data.share.download.dir.prefix");
    ...
    private void dispatchMessages(JSONObject response) throws Exception {
        ...
        for (Object item : messages) {
            ...
            String serviceName = TransferStringHelper.getWorkflowCode(fileName, "", 2);
            String cityNo = TransferStringHelper.getWorkflowCode(fileName, "", 3);
            ...
            if (fileName.endsWith(".verf")) {
                fileType = TransferDataFileCategory.VERF.getCode();
                fileDir = TransferStringHelper.addEndSeparator(downloadRoot) + txDate + "/VERF/"
                        + sendSysCode + "/" + unitName + "/" + createTime;
            } else {
                fileType = TransferDataFileCategory.DATA.getCode();
                fileDir = TransferStringHelper.addEndSeparator(downloadRoot) + txDate + "/"
                        + sendSysCode + "/" + unitName + "/" + createTime;
            }

            String messageAckId = message.getString("messageAckId");
            TransferDataShareMessage entity = buildEntity(message, fileName, serviceName, sendSysCode,
    ...
    private TransferDataShareMessage buildEntity(JSONObject message, String fileName, String serviceName,
        ...
        TransferDataShareMessage entity = new TransferDataShareMessage();
     ...
     * 依据消息 File_id 做幂等判断。
     */
    private boolean isDuplicated(TransferDataShareMessage entity) {
    ...
    private String resolveSendSysCode(JSONObject message, String fileName) {
        ...
        sendSysCode = TransferStringHelper.getWorkflowCode(fileName, "", 1);
        String mappingJson = TransferAppPropertiesUtil.getProperty("interface.filename.syscode.map");
    ...
    private String resolveTxDate(String fileName, String serviceName, String unitName) throws Exception {
        String txDate = TransferStringHelper.getWorkflowCode(fileName, "", 4);
