...
@EnableScheduling
@Slf4j
public class TransferProcessFileJob implements TransferJob {

    private final TransferTransferActionService actionService;
    private final TransferDataShareMessageMapper shareMsgMapper;

    public TransferProcessFileJob(TransferTransferActionService actionService, TransferDataShareMessageMapper shareMsgMapper) {
    ...
    public synchronized void run() {
        List<TransferDataShareMessage> pending = shareMsgMapper.selectPendingProcess();
        ...
        for (TransferDataShareMessage msg : pending) {
            ...
            params.put("processFlag", TransferFlowStatus.RUNNING);
            params.put("oldFlag", TransferFlowStatus.READY);
