 ...
 * TransferByteEncodingDetector 的概率算法（gb2312_probability 等）需对照原工程补全。
 */
@Slf4j
public class TransferFileEncodingDetector {
     ...
     * @return Java 编码名
     */
    public static String getJavaEncode(String filePath, String encode) {        TransferByteEncodingDetector probe = new TransferByteEncodingDetector();
        TransferByteEncodingDetector resolver = new TransferByteEncodingDetector();
        String fileCode = resolver.javaname[probe.detect(new File(filePath))];

        String mappingJson = TransferAppPropertiesUtil.getProperty("interface.filepath.filecode.map");
        if ("".equals(encode)) {
        ...
        log.info("文件: " + filePath + " TransferFileEncodingDetector>>>>>>>>>>>>>>>>>>>>>>>>>>" + fileCode);
        return fileCode;
    }
 ...
 * 字节流编码探测器。
 */
class TransferByteEncodingDetector extends TransferEncodingTable {
    ...
    public TransferByteEncodingDetector() {
        super();
        debug = false;
    ...
    public static void main(String[] options) {
        TransferByteEncodingDetector detector = new TransferByteEncodingDetector();
