 ...
 */
public class TransferHdfsFileClient implements AutoCloseable {

    private static final Logger log = LoggerFactory.getLogger(TransferHdfsFileClient.class);
    ...
    public TransferHdfsFileClient() {
        Configuration configuration = new Configuration();
        System.setProperty("java.security.krb5.conf", TransferAppPropertiesUtil.getProperty("hadoop.krb.path"));
        configuration.set("hadoop.security.authentication", "kerberos");
        configuration.set("fs.hdfs.impl", "org.apache.hadoop.hdfs.DistributedFileSystem");
        configuration.set("fs.defaultFS", TransferAppPropertiesUtil.getProperty("hadoop.name.node"));
        ...
        configuration.set("dfs.namenode.rpc-address." + NAMESERVICE + ".nn1",
                TransferAppPropertiesUtil.getProperty("hadoop.name.host1"));
        configuration.set("dfs.namenode.rpc-address." + NAMESERVICE + ".nn2",
                TransferAppPropertiesUtil.getProperty("hadoop.name.host2"));
        configuration.set("dfs.client.failover.proxy.provider." + NAMESERVICE, FAILOVER_PROVIDER);
        try {
            UserGroupInformation.setConfiguration(configuration);
            UserGroupInformation.loginUserFromKeytab(
                    TransferAppPropertiesUtil.getProperty("hadoop.principal.name"),
                    TransferAppPropertiesUtil.getProperty("hadoop.keytab.path"));
