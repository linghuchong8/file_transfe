...
import com.transfer.medical.util.MedicalConnection;
 ...
 * Medical 文件传送服务。
 *
 * 封装 Medical 连接建立、文件上传与断链重连逻辑。
 *
 * @author wanglei
 */
@Service
@Slf4j
public class MedicalPutService {
    ...
    /** Medical 上传动作。 */
    @FunctionalInterface
    private interface MedicalPutAction {
     ...
     * 执行一次上传，处理断链重连。
     */
    private int transfer(MedicalConnection connection, EFTP_APP app, int handle, MedicalPutAction action) {
        int result = -1;
        try {
            if (handle <= 0) {
     ...
     * 连接失败时循环重试，直到成功。
     */
    private int connectWithRetry(MedicalConnection connection, EFTP_APP app) {
        int handle = connection.getConnection(app);
        int attempt = 0;
        while (handle <= 0) {
            try {
