...
import com.transfer.medical.util.MedicalConstants;
...
/**
 * 医疗数据交换（EDB）客户端 HTTP 交互服务。
 *
 * 负责消息接收、文件下载/上传、消息回执与插件探测等 HTTP 调用。
 */
@Slf4j
public class MedicalHttpClientService {
    ...
    public MedicalHttpClientService(String ipList) {
    ...
    public boolean downloadFile(String jsonInputStr, String backupPluginPath, String flag, String fileName) {
        ...
        log.error("下载重试 {} 次仍失败, 文件 {}", MedicalConstants.MAX_RETRIES, fileName);
        return false;
    }
    ...
    public boolean uploadFile(String jsonInputStr) {
        ...
        while (attempt <= MedicalConstants.MAX_RETRIES) {
            ...
            attempt++;
            if (attempt <= MedicalConstants.MAX_RETRIES) {
                log.warn("地址不可用, 上传失败, 重试 {} 次中的第 {} 次", MedicalConstants.MAX_RETRIES, attempt);
            }
        }
        log.error("上传重试 {} 次仍失败, 响应 {}", MedicalConstants.MAX_RETRIES, response);
        return false;
    }
